...
Page({
   ...
   */
  loadIndex: function () {
    var banner = util.getStorageStudyfox('banner')
    var ad = util.getStorageStudyfox('ad')
    var goods = util.getStorageStudyfox('goods')
    //先判断缓存
    if (banner && ad && goods){
      ...
      util.wxRequest(url, params, data => {
        if (data.code == 200) {
          ...
          util.setStorageStudyfox('banner', data.banner)
          util.setStorageStudyfox('ad', data.ad)
          util.setStorageStudyfox('goods', data.goods)
        }
      }, data => { }, data => { })
    }
